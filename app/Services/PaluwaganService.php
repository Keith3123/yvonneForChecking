...
class PaluwaganService
{
     ...
     * @param int         $startDay
     * @param string|null $startTime   HH:MM format, e.g. "09:00"
     */
    public function joinPaluwagan(
        int    $customerID,
        int    $packageID,
        int    $startMonth,
        int    $startDay   = 15,
        ...
        return DB::transaction(function () use ($customerID, $packageID, $startMonth, $startDay, $startTime) {

            // Block duplicate: same customer + package + month + day (+ time if provided)
            $query = PaluwaganEntry::where('customerID', $customerID)
     ...
     *  - Each schedule due date = 15th of each month.
    ...
    public function generateSchedules(PaluwaganEntry $entry, ?PaluwaganPackage $package = null): void
    {
        ...
        // If time is specified, incorporate it into the release datetime
        if ($entry->startTime) {
            try {
                [$hour, $minute] = explode(':', $entry->startTime);
                $releaseDate->setTime((int)$hour, (int)$minute, 0);
            } catch (\Exception $e) {
                // fallback: keep startOfDay
            }
        }
